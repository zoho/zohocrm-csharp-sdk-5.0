...
namespace Com.Zoho.API.Authenticator.Store
{
    ...
    public class DBStore : ITokenStore
    {
        ...
        public void SaveToken(IToken token)
        {
            ...
            try
            {
                using (MySqlConnection connection = new MySqlConnection(this.connectionString))
                {
                    ...
                    if(rowsAffected == 0)
                    {
                        string createquery = "insert into " + this.tableName + "(id,user_name,client_id,client_secret,refresh_token,access_token,grant_token,expiry_time,redirect_url) values (@id, @user_name, @client_id, @client_secret, @refresh_token, @access_token, @grant_token, @expiry_time, @redirect_url);";
                    ...
                    using (MySqlCommand statement = new MySqlCommand(query, connection))
                    {
                        using (MySqlDataReader result = statement.ExecuteReader())
                        {
                            while (result.Read())
                            {
                                ...
                                if (name != null)
                                {
                                    token.UserSignature = new UserSignature(name);
                                }
        ...
        private void SetMergeData(OAuthToken oauthToken, MySqlDataReader result)
	    {
            ...
            {
                string expiresIn = result[Constants.EXPIRES_IN].ToString();
                if (expiresIn != null)
                {
                ...
                using (MySqlConnection connection = new MySqlConnection(this.connectionString))
                {
                    ...
                    using (MySqlCommand statement = new MySqlCommand(query.ToString(), connection))
                    {
                        using (MySqlDataReader result = statement.ExecuteReader())
                        {
						    do
						    {
							    int? max = (int)result[Constants.ID];
							    if (max != null)
							    {
								    return (int)(max + 1);
							    }
