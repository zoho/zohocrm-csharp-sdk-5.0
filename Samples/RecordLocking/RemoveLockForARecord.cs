...
{
    public class RemoveLockForARecord
	{
		public static void RemoveLockForARecord_1(long recordId, string moduleName, long lockId)
		{
			RecordLockingOperations recordLockingOperations = new RecordLockingOperations();
			APIResponse<ActionHandler> response = recordLockingOperations.RemoveLockForARecord(lockId, recordId, moduleName);
			if (response != null)
			{
		...
		public static void Call()
		{
			try
			{
				...
				long lockId = 3477062002l;
                RemoveLockForARecord_1(recordId, moduleAPIName, lockId);
			}
			catch (Exception e)
			{
