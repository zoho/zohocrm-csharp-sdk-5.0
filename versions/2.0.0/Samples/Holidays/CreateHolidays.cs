...
using BusinessHoliday = Com.Zoho.Crm.API.Holidays.BusinessHoliday;
using CreateBusinessHoliday = Com.Zoho.Crm.API.Holidays.CreateBusinessHoliday;
using CreateShiftHoliday = Com.Zoho.Crm.API.Holidays.CreateShiftHoliday;
...
using ShiftHoliday = Com.Zoho.Crm.API.Holidays.ShiftHoliday;
...
namespace Samples.Holidays
{
	...
	{
		public static void CreateHolidays_1()
		{
			HolidaysOperations holidaysoperations = new HolidaysOperations("4402480020813");
			// when type is business_holiday
			CreateBusinessHoliday request = new CreateBusinessHoliday();
			List<BusinessHoliday> holidays = new List<BusinessHoliday>();
			BusinessHoliday holiday = new BusinessHoliday();
			holiday.Name = "Holiday 2";
			holiday.Date = new DateTime(2023, 8, 22).Date;
			holiday.Type = new Choice<string>("business_holiday");
			holidays.Add (holiday);
			request.Holidays = holidays;
			// when type is Shift_holiday
			CreateShiftHoliday request1 = new CreateShiftHoliday();
			List<ShiftHoliday> shiftholidays = new List<ShiftHoliday>();
			...
			shiftholiday.ShiftHour = shifthour;
			shiftholiday.Name = "shiftholiday";
			shiftholiday.Type = new Choice<string>("shift_holiday");
			shiftholiday.Date = new DateTime(2023, 8, 22).Date;
			shiftholidays.Add (shiftholiday);
			request1.Holidays = shiftholidays;
			APIResponse<ActionHandler> response = holidaysoperations.CreateHolidays(request1);
			if (response != null)
			{
