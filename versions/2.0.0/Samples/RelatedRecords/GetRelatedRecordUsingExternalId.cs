...
namespace Samples.Relatedrecords
{
	...
	{
		public static void GetRelatedRecordUsingExternalId_1(string moduleAPIName, string externalValue, string relatedListAPIName, string externalFieldValue, string destinationFolder)
		{
						...
						foreach (Com.Zoho.Crm.API.Record.Record record in records)
						{
							...
							foreach (KeyValuePair<string, object> entry in record.GetKeyValues())
							{
								...
								if (value is IList)
								{
									...
									if (dataList.Count > 0)
									{
                                        ...
                                        else if (dataList[0].GetType().FullName.Contains("Choice"))
                                        {
                                            ...
                                            Console.WriteLine("RelatedRecord values");
                                ...
                                else if (value is Consent)
								{
									...
									Console.WriteLine ("RelatedRecord Consent ID: " + consent.Id);
		...
		public static void Call()
		{
			try
			{
                ...
                GetRelatedRecordUsingExternalId_1(moduleAPIName, externalValue, relatedListAPIName, externalFieldValue, destinationFolder);
			}
			catch (Exception e)
			{
