...
namespace Com.Zoho.API.Authenticator.Store
{
    ...
    public class FileStore : ITokenStore
    {
        ...
        private List<string> headers = new List<string>() { Constants.ID, Constants.USER_NAME, Constants.CLIENT_ID, Constants.CLIENT_SECRET, Constants.REFRESH_TOKEN, Constants.ACCESS_TOKEN, Constants.GRANT_TOKEN, Constants.EXPIRY_TIME, Constants.REDIRECT_URL };
            ...
            try
            {
                ...
                for (int index = 1; index < allContents.Count; index++)
                {
                    ...
                    if(nextRecord.Length > 1)
                    {
                        ...
                        {
                            SetMergeData(oauthToken, nextRecord);
                            allContents.Insert(index, string.Join(",", SetToken(oauthToken)));
                            break;
                        }
                    }
                    else
